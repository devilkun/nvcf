...
#
# This is the official Apache Cassandra cassandra-env.sh (see the license
# block below) with one NVIDIA/NCP addition appended at the end: setting the
# rack in cassandra-rackdc.properties from the StatefulSet pod ordinal, for
# GossipingPropertyFileSnitch usage.
#
# This file is sourced by /opt/cassandra/bin/cassandra as the cassandra user,
# immediately before the JVM is launched. The official docker-entrypoint.sh
# rewrites cassandra-rackdc.properties from CASSANDRA_RACK earlier, before
# this file is sourced, so setting CASSANDRA_RACK here would arrive too late.
# The rack-from-pod block below writes cassandra-rackdc.properties directly
# for that reason.
...
# Custom NCP addition: extract pod ordinal from hostname and set the rack in
# cassandra-rackdc.properties based on pod number, for GossipingPropertyFileSnitch
# usage. This mirrors the previous Bitnami-based image's CASSANDRA_RACK
# override, ported to the official image's layout: the official
# docker-entrypoint.sh already rewrote cassandra-rackdc.properties from
# CASSANDRA_RACK before this file was sourced, so the rack must be applied
# to the properties file directly here instead of via an env var. This file
# is sourced by /opt/cassandra/bin/cassandra, whose shebang is /bin/sh
# (dash), so the block below is written in POSIX sh, not bash.
if [ -n "${HOSTNAME:-}" ]; then
    ...
    CASSANDRA_RACKDC_FILE="${CASSANDRA_CONF:-/etc/cassandra}/cassandra-rackdc.properties"
    if [ -f "${CASSANDRA_RACKDC_FILE}" ]; then
        sed -i.bak -E "s/^(rack=).*/\1${RACK_NAME}/" "${CASSANDRA_RACKDC_FILE}"
        rm -f "${CASSANDRA_RACKDC_FILE}.bak"
    fi

    echo "Pod ${HOSTNAME} (number: ${POD_NUMBER}) assigned to rack: ${RACK_NAME}"
fi
