...
| --- | --- | --- |
| One-click CLI installation | You want the fastest fresh install and cluster registration path. | [Quickstart](./quickstart.md) |
...
[Quickstart](./quickstart.md) and [Gateway Routing](./gateway-routing.md).
...
3. Install the self-hosted control plane. Use the [Quickstart](./quickstart.md) for a one-click fresh install, [Helmfile Installation](./helmfile-installation.md) for manual Helmfile operations, or [Standalone Deployment](./standalone-deployment.md) for chart-by-chart installation.

4. Register a GPU cluster and install the NVIDIA Cluster Agent. The quickstart performs this step. For manual installation paths, see [Self-Managed Clusters](./cluster-management/self-managed.md).
