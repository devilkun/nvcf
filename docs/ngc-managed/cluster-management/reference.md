...
1. **Shared parameters** (top-level) — Apply to **all deployment modes**. These control the
...
2. **`helmManaged.*`** — Only used when `ngcConfig.clusterSource` is `"helm-managed"`.
...
```yaml
 ngcConfig:
   username: '$oauthtoken'
   serviceKey: ""                   # NGC Cluster Key or API key
