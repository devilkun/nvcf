...
/**
 * Client for interacting with Amazon ECR Public Registry.
 * Handles container image URL parsing and validation using AWS ECR Public DescribeImages API.
 */
@Slf4j
public class EcrPublicContainerRegistryClient {

    /**
     * Expected format: public.ecr.aws/{registry_alias}/{repository}:{tag}
     * or public.ecr.aws/{registry_alias}/{repository}@{digest}
     */
    public static final Pattern ECR_PUBLIC_CONTAINER_IMAGE_URL_PATTERN = Pattern.compile(
            "^public\\.ecr\\.aws/(?<registryAlias>[a-z0-9](?:[a-z0-9._-]*[a-z0-9])?)/(?<repository>[a-z0-9](?:[a-z0-9._/-]*[a-z0-9])?)(?::(?<tag>[A-Za-z0-9._-]+))?(?:@(?<digest>sha256:[a-f0-9]{64}))?$"
    );
    ...
    public EcrPublicContainerRegistryClient(
        ...
        var webClient = getEcrWebClientBuilder(webClientBuilder, timeout)
                .baseUrl(toBaseUrlForEcrPublicRegistry(containerRegistryUrl))
                .build();
    ...
    /**
     * Validates a container image by checking its existence in the ECR Public registry using DescribeImages API.
     *
     * @param containerImageUrl The container image URL to validate (format: public.ecr.aws/{registry_alias}/{repository}:{tag})
    ...
    public void validateContainerImage(String containerImageUrl, String base64ApiKey) {
        var components =
                parsePublicArtifactUrl(containerImageUrl, ECR_PUBLIC_CONTAINER_IMAGE_URL_PATTERN);
