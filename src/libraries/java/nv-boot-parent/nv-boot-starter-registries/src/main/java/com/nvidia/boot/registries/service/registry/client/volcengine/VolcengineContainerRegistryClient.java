...
/**
 * Client for interacting with Volcengine Container Registry.
 * Handles container image URL parsing, signature generation, and validation.
 */
@Slf4j
public class VolcengineContainerRegistryClient {

    /**
     * Expected format: [registry]-[region].cr.volces.com/[namespace]/[repository]:[tag]
     */
    public static final Pattern CONTAINER_IMAGE_URL_PATTERN = Pattern.compile(
            "^(?<registry>[\\w-]+)-(?<region>(?:cn|ap)-[\\w-]+)\\.cr\\.volces\\.com/(?<namespace>[\\w._-]+)/(?<repository>[\\w._/-]+)(?::(?<tag>[\\w._-]+))?$"
    );
    ...
    public VolcengineContainerRegistryClient(
        ...
        var webClient = createVolcengineWebClient(webClientBuilder, toBaseUrl(containerRegistryUrl), timeout);
    ...
    /**
     * Validates a container image by checking its existence in the Volcengine registry.
     *
