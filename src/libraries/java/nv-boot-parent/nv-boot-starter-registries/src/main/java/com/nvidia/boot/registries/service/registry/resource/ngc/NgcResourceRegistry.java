...
package com.nvidia.boot.registries.service.registry.resource.ngc;
...
@Slf4j
public class NgcResourceRegistry implements ResourceRegistry {
    ...
    @Override
    public long fetchSize(ArtifactDetails resource, String secret) {
        var apiKey = NgcRegistryUtils.getApiKey(secret);
        var size = ngcArtifactRegistryClient.fetchResourceSize(resource.url(), apiKey);
        log.trace(MESG_FETCHED_SIZE_ARTIFACT_URLS, resource.name(), resource.url());
        return size;

    }

    @Override
    public Artifact fetchArtifact(ArtifactDetails resource, String secret) {
        var apiKey = NgcRegistryUtils.getApiKey(secret);
        var preSignedModelURLs =
                ngcArtifactRegistryClient.getPreSignedArtifactURLs(resource.url(), apiKey);
        log.trace(MESG_FETCHED_PRESIGNED_ARTIFACT_URLS, resource.name(), preSignedModelURLs);
        return new Artifact(resource.name(),
                            resource.version(),
    ...
    @Override
    public void validateArtifact(ArtifactDetails resource, String secret) {
        ngcArtifactRegistryClient.validateArtifact(resource.url(),
