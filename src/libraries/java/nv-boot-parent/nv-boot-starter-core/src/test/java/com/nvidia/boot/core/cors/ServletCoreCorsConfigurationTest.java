...
class ServletCoreCorsConfigurationTest {
    ...
    @Test
    void reactiveCorsBeanIsNotLoaded() {
        contextRunner.run(context -> {
            assertThat(context).doesNotHaveBean(ReactiveCoreCorsConfiguration.class);
        });
    }

    @Test
    void corsFilterBeanIsRegistered() {
        contextRunner.run(context -> {
    ...
    void corsConfigHasExpectedSettings() {
        var config = ServletCoreCorsConfiguration.corsConfig();
        ...
        assertThat(config.getMaxAge()).isEqualTo(86400L); // 1 day in seconds
    }
}
