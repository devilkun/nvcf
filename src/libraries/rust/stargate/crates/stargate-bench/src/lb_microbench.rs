...
use stargate::load_balancer_state::{RoutedClusterSnapshot, RoutingTargetKey};
...
fn config_for_scenario(scenario: LbMicrobenchScenario) -> LoadBalancerAlgorithmConfig {
    ...
    config.seed = Some("lb-microbench-seed".to_string());
    let is_pulsar = matches!(
        scenario,
        LbMicrobenchScenario::Pulsar | LbMicrobenchScenario::PulsarOneExcluded
    );
    config.require_cache_affinity_key = Some(is_pulsar);
    config.require_input_tokens = Some(is_pulsar);
    config.require_kv_metrics = Some(is_pulsar);
    config.ttft_bucket_size_ms = Some(1_000_000);
    config.n = Some(2);
    config.max_queued = Some(64);
    if matches!(
        scenario,
        LbMicrobenchScenario::GroqMultiregionAffinity
            | LbMicrobenchScenario::GroqMultiregionAffinityOneExcluded
        ...
        config.cache_affinity_virtual_nodes = Some(150);
        config.cache_affinity_backend_selection_count = Some(2);
    }
    if matches!(
        scenario,
        LbMicrobenchScenario::GroqMultiregionIgnoreQueue
        ...
        config.ignore_queue_time = Some(true);
    }
    if matches!(
        scenario,
        LbMicrobenchScenario::GroqMultiregionRttOnly
        ...
        config.ignore_input_processing_time = Some(true);
    }
    config
}
