...
def _go_oci_image_impl(name, visibility, binary, base, entrypoint, binary_path, registry, extra_registries, tags):
    ...
    if binary_path:
        ...
        pkg_files(
            ...
            # Without an explicit mode, pkg_files packages the binary
            # as 0644 and the container fails to start with "permission
            # denied" on the entrypoint. Mirrors helm-reval !365.
        ...
        pkg_tar(
            name = layer_name,
            srcs = [":" + files_name],
            # Layer visibility matches the image's so the calling
            # BUILD.bazel can wire a sh_test against the layer for
            # entrypoint exec-bit regression (helm-reval !365 pattern).
