...
- `github.com/nvidia-lpu/*` uses full module trees under `nvidia-lpu-vendor/` via `replace` in `go.mod`. `bin/sync-nvidia-lpu-vendor.sh` reads `go.mod`, downloads each module into the cache, and copies the tree into `nvidia-lpu-vendor/`. `mise run bootstrap` runs `bootstrap:nvidia-lpu-vendor` first (so replace targets exist), then `bootstrap:go-deps`, then `bootstrap:tokenizers`. Commit `nvidia-lpu-vendor/` when these vendored module versions change.
