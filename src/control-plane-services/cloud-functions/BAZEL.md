...
Developers edit the root `MODULE.bazel`. Cloud Functions does not have a nested
...
Maven build, publish Cloud Functions artifacts, or make the Bazel outputs
...
- Java 25. The root `.bazelrc` selects the local Java 25 JDK for compilation
  and runtime.
- Docker is required for the current `nvcf-core` and `nvcf-service`
  integration tests.
...
mirrors. Bazel applies it automatically through `.bazelrc`; Cloud Functions should
...
It does not contain `nv-boot-parent`, `nvcf-core`, or `nvcf-service`; those are
...
it reaches Cloud Functions compilation or runtime only when a BUILD dependency
edge selects it.
...
nv-boot and Cloud Functions: both are ordinary directories in the same Bazel
module.
...
Cloud Functions intentionally pins gRPC runtime and code generation to 1.63.0.
...
Build all Cloud Functions targets:
...
Build the main `nvcf-core` library:

```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  build //src/control-plane-services/cloud-functions/nvcf-core:nvcf_core \
  --verbose_failures
```
...
```text
bazel-bin/src/control-plane-services/cloud-functions/nvcf-core/libnvcf_core.jar
```

Build the `nvcf-core` test-fixtures target consumed by `nvcf-service` tests:

```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  build //src/control-plane-services/cloud-functions/nvcf-core:nvcf_core_test_fixtures \
  --verbose_failures
```
...
Build the private `nvcf-service` application classes target for compile-only
feedback:

```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  build //src/control-plane-services/cloud-functions/nvcf-service:app_classes \
  --verbose_failures
```
...
```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  build //src/control-plane-services/cloud-functions/nvcf-service:app \
  --verbose_failures
```
...
```text
bazel-bin/src/control-plane-services/cloud-functions/nvcf-service/app.jar
```
...
META-INF/maven/com.nvidia.nvcf/nvcf-service-oss/pom.properties
```
...
```bash
java -jar bazel-bin/src/control-plane-services/cloud-functions/nvcf-service/app.jar \
...
Resolve the real Bazel output directory and use the `nvcf-service` output
...
docker build \
  -f src/control-plane-services/cloud-functions/nvcf-service/Dockerfile \
  --build-arg APP_JAR=app.jar \
  -t cloud-functions-nvcf-service:bazel \
  "${BAZEL_BIN_DIR}/src/control-plane-services/cloud-functions/nvcf-service"
```
...
docker run --rm \
  --name nvcf-service \
  --mount "type=bind,source=$(pwd)/src/control-plane-services/cloud-functions,target=/home/app,readonly" \
  -e SPRING_PROFILES_ACTIVE=local \
  -e SPRING_CASSANDRA_CONTACT_POINTS=host.docker.internal \
  -p 8080:8080 \
  -p 9090:9090 \
  -p 8181:8181 \
  cloud-functions-nvcf-service:bazel
```
...
Run the full `nvcf-core` test target:

```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  test //src/control-plane-services/cloud-functions/nvcf-core:tests \
  --cache_test_results=no \
  --test_output=errors \
  --test_env=DOCKER_HOST \
  --test_env=DOCKER_TLS_VERIFY \
...
The full `nvcf-core` test target currently runs JUnit through
`org.junit.platform.console.ConsoleLauncher`.

## Test `nvcf-service`

Run the `nvcf-service` Spring Boot smoke/integration test target:

```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  test //src/control-plane-services/cloud-functions/nvcf-service:tests \
  --cache_test_results=no \
  --test_output=errors \
  --test_env=DOCKER_HOST \
  --test_env=DOCKER_TLS_VERIFY \
...
The test target starts the `App` context with the `nvcf-core` test fixture
...
```text
src/control-plane-services/cloud-functions/nvcf-core/src/test/java/com/nvidia/nvcf/service/apikeys/ApiKeyValidationResultTest.java
```

becomes:

```text
com.nvidia.nvcf.service.apikeys.ApiKeyValidationResultTest
```
...
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  ...
  --test_env=DOCKER_TLS_CERTDIR \
  --test_env=DOCKER_CERT_PATH \
  --test_arg=--exclude-classname='^(?!com\.nvidia\.nvcf\.service\.apikeys\.ApiKeyValidationResultTest$).*$' \
  --verbose_failures
```
...
```text
bazel-testlogs/src/control-plane-services/cloud-functions/<module>/tests/
```
...
1. Cloud Functions' Bazel target generates and checks the complete third-party
...
Cloud Functions derives its actual shipped coordinates from the jars nested in:

```text
//src/control-plane-services/cloud-functions/nvcf-service:app
```

The root owns the shared rule and generator under `//rules/java` and
`//tools/bazel/java`. Cloud Functions owns:

```text
src/control-plane-services/cloud-functions/NOTICE
src/control-plane-services/cloud-functions/notice_metadata.json
```

Cloud Functions does not need `notice_roots.json`: the executable `app.jar` is the
...
metadata. The generated Cloud Functions NOTICE is complete and standalone; it is
...
```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  run //src/control-plane-services/cloud-functions:generate_notice -- --write
```
...
```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  run //src/control-plane-services/cloud-functions:generate_notice -- \
...
```bash
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  test //src/control-plane-services/cloud-functions:notice_check_test \
  --cache_test_results=no \
  --test_output=errors
```
...
bazel --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" \
  build \
  //src/control-plane-services/cloud-functions:cloud_functions_runtime_inventory \
  //src/control-plane-services/cloud-functions:osrb_dependency_delta

cat bazel-bin/src/control-plane-services/cloud-functions/runtime_inventory.json
cat bazel-bin/src/control-plane-services/cloud-functions/osrb_dependency_delta.json
cat bazel-bin/src/control-plane-services/cloud-functions/osrb_dependency_delta.md
```

The delta compares exact versioned Cloud Functions runtime coordinates with the
...
The detector models Cloud Functions, nv-boot, and shared Java changes, but current
...
or shared Java tooling must also select Cloud Functions as a reverse-dependency
validation target.
...
The component-local `bazel-java-ci.json` registers Cloud Functions with the root
...
Cloud Functions is part of the monorepo root Bazel module. CI invokes Bazel from
the repository root with:

```text
//src/control-plane-services/cloud-functions/...
```
...
entering Cloud Functions and expecting a nested `MODULE.bazel`. All Java components
...
```text
bazel-cloud-functions-verification-<run-attempt>
```
