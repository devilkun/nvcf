...
@Tag(name = "Cross-Account Asset Management for NVIDIA Super Admins",
        description = """
                ...
                Defines Asset Management endpoints for NVIDIA Super Admins. All the endpoints
                 defined in this API require a bearer token in the HTTP Authorization header with
                 'admin:invoke_function' scope."""
)
public class XAccountAssetController {
    ...
    @PostMapping(value = "/assets", consumes = APPLICATION_JSON_VALUE)
    @Operation(
            ...
            description = """
                    Creates a unique id representing an asset and a pre-signed URL to upload the
                     ...
                     bearer token with 'admin:invoke_function' scope in the HTTP Authorization
                     header.
                     """
    )
    @PreAuthorize("hasAuthority('admin:invoke_function')")
    ...
    @Operation(
            summary = "List Assets",
            description = """
                    List assets owned by the current NVIDIA Cloud Account. Requires a bearer
                     token with 'admin:invoke_function' scope in the HTTP Authorization header.
                    """
    )
    @PreAuthorize("hasAuthority('admin:invoke_function')")
    public ListAssetsResponse getAssets(@Parameter(description = NCA_ID_DESCRIPTION, required = true)
                                              @PathVariable String ncaId,
                                              Authentication authentication) {
        NvcfUtils.addTagsToCurrentSpan(tracer, Map.of(SPAN_TAG_NCA_ID, ncaId));
        rateLimiterService.verifyLimits(ncaId);
    ...
    @Operation(
            ...
            description = """
                    Returns details for the specified asset-id belonging to the current NVIDIA
                     Cloud Account. Requires a bearer token with  'admin:invoke_function' scope
    ...
    @Hidden
    @GetMapping("/assets/{assetId}/content-redirect")
    @Operation(
    ...
    @Operation(
            ...
            description = """
                    Deletes asset belonging to the current NVIDIA Cloud Account. Requires a
                     bearer token with 'admin:invoke_function' scope in the HTTP Authorization
                     header.""",
