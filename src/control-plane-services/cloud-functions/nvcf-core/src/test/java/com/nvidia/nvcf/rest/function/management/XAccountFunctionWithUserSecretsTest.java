...
class XAccountFunctionWithUserSecretsTest {
    ...
    Stream<Arguments> createFunctionWithSecretArgs() {
        ...
        return Stream.of(
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                             ...
                             HttpStatus.OK),
                // long secret name - exceeding 512 length
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                                                             List.of(ADMIN_SCOPE_REGISTER_FUNCTION,
                                                          ADMIN_SCOPE_LIST_FUNCTIONS),
                                                             100),
                             Set.of(SecretDto.builder()
                                            .name(StringUtils.repeat("secret1", 512))
                                            .value(new StringNode("value1")).build()),
                             null,
                             HttpStatus.BAD_REQUEST),
                // empty secret value
    ...
    @ParameterizedTest
    @MethodSource("createFunctionWithSecretArgs")
    ...
    Stream<Arguments> listFunctionWithSecretsArgs() {
        return Stream.of(
                Arguments.of(ADMIN_SCOPE_LIST_FUNCTIONS_DETAILS, null),
                Arguments.of(ADMIN_SCOPE_LIST_FUNCTIONS_DETAILS, Boolean.TRUE),
                Arguments.of(ADMIN_SCOPE_LIST_FUNCTIONS_DETAILS, Boolean.FALSE),
                Arguments.of(ADMIN_SCOPE_LIST_FUNCTIONS, Boolean.TRUE),
                Arguments.of(ADMIN_SCOPE_LIST_FUNCTIONS, Boolean.FALSE)
        );
    };
    ...
    void shouldGetFunctionDetailsWithSecretsUsingVersionId(
        ...
        var functionDto = responseBody.function();
        assertThat(functionDto).isNotNull();
        if (listFunctionsScope.equals(ADMIN_SCOPE_LIST_FUNCTIONS_DETAILS)) {
    ...
    void shouldDeleteFunctionWithSecrets() {
        ...
        var deleteRequestEntity =
                RequestEntity.delete(URI.create("/v2/nvcf/accounts/"
                                                        + TEST_NCA_ID + "/functions/" + functionId
                                                        + "/versions/" + versionId))
                        .header("Authorization", "Bearer " + token)
                        .build();
    ...
    void shouldListPublicFunctionWithSecrets(Object tokenSupplier,
        ...
        // Create a function associated with public account with secrets in TEST_PUBLIC_FUNCTION_CLIENT_ID
