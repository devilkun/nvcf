...
import com.nvidia.nvcf.service.reval.RevalStubService.RevalValidateResponse;
...
class RevalSerializationTest {
    ...
    void testRequestSerialization()  {
        ...
        var expectedJson = """
                {"configuration":{"key":"value","key2":2},"helmChart":"my_helm_chart"}""";

        assertThat(requestJson).isEqualTo(expectedJson);
    }
    ...
    void testResponseDeserialization() {
        var json = """
            {"valid":true,"validationErrors":["Error 1","Error 2"],"internalErrors":["Internal Error 1"]}
            """;
        ...
        assertThat(expectedValidationResult).isEqualTo(validationResult);
    }
}
