...
class FunctionWithRatelimitTest {
    ...
    Stream<Arguments> listFunctionWithRatelimitArgs() {
        return Stream.of(
                Arguments.of(SCOPE_LIST_FUNCTIONS_DETAILS),
                Arguments.of(SCOPE_LIST_FUNCTIONS_DETAILS),
                Arguments.of(SCOPE_LIST_FUNCTIONS_DETAILS),
                Arguments.of(SCOPE_LIST_FUNCTIONS),
                Arguments.of(SCOPE_LIST_FUNCTIONS)
                        );
    }
    ...
    void shouldListFunctionsWithRatelimitByAccount(
        ...
        var token = MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                 ...
                                                 listFunctionsScope),
                                                    100);
        // Create a function with ratelimit policy in TEST_NCA_ID
    ...
    @ParameterizedTest
    @MethodSource("listFunctionWithRatelimitArgs")
    void shouldListFunctionsWithPerUserRate(String listFunctionsScope) {
        var token = MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                    List.of(SCOPE_REGISTER_FUNCTION,
                                                            listFunctionsScope),
                                                    100);
        ...
        var listRequestEntity = RequestEntity.get(URI.create("/v2/nvcf/functions"))
                .header("Authorization", "Bearer " + token)
                .build();
        var listResponseEntity = testRestTemplate.exchange(listRequestEntity, ListFunctionsResponse.class);
        var responseBody = listResponseEntity.getBody();
        ...
        var functionDto = responseBody.functions().getFirst();
        assertThat(functionDto.rateLimit()).isNotNull();
        assertThat(functionDto.rateLimit().rateLimit()).isEqualTo("10-S");
        assertThat(functionDto.rateLimit().perUserRate()).isEqualTo("2-S");
    }
    ...
    Stream<Arguments> publicFunctionsListRatelimit() {
        return Stream.of(
                ...
                Arguments.of((Supplier<String>) () -> {
                             ...
                             false),
                // Create token using OAuth2 Client to TEST_PUBLIC_FUNCTION_NCA_ID to list
    ...
    void shouldListPublicFunctionWithRatelimit(Object tokenSupplier,
        ...
        // Create a function associated with public account with ratelimit in TEST_PUBLIC_FUNCTION_NCA_ID
        ...
        // Delete functions before deleting account.
