...
class FunctionSecretManagementTest {
    @Autowired
    ...
    Stream<Arguments> updateFunctionSecretArgs() {
        ...
        var jwtCases = Stream.of(
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                             ...
                             Set.of(SecretDto.builder()
                                    ...
                                    SecretDto.builder()
                                            ...
                                            .value(secretJsonNodeValue).build()),
                             HttpStatus.NO_CONTENT),
                // duplicate secrets (case sensitive)
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                             List.of(SCOPE_UPDATE_SECRETS),
                                                             100),
                             Set.of(SecretDto.builder().name("secret1")
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                             ...
                             Set.of(SecretDto.builder().name("")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.BAD_REQUEST),
                // Secret name - exactly 512 in length
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                             List.of(SCOPE_UPDATE_SECRETS),
                                                             100),
                             Set.of(SecretDto.builder()
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                             ...
                             Set.of(SecretDto.builder().name("*secret1*-\"")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.BAD_REQUEST),
                // wrong nca id/function doesn't belong to nca id
        ...
        var apiKeyCases = Stream.of(
                // Authorized to update secrets for any private functions of TEST_NCA_ID account.
                Arguments.of((Supplier<String>) () -> {
                             ...
                             Set.of(SecretDto.builder().name("secret1")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.NO_CONTENT),
                // Authorized to update secrets for specific function version of TEST_NCA_ID account
                Arguments.of((Supplier<String>) () -> {
                                 setResponse(TEST_NCA_ID, TEST_OWNER_ID,
                             ...
                             Set.of(SecretDto.builder().name("secret1")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.NO_CONTENT),
                // Authorized to update secrets for all versions of specific function of
                // TEST_NCA_ID account.
                Arguments.of((Supplier<String>) () -> {
                                 setResponse(TEST_NCA_ID, TEST_OWNER_ID,
                                             List.of(new Resource("function",
                             ...
                             Set.of(SecretDto.builder().name("secret1")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.NO_CONTENT),
                // Attempt to update secrets for a function of TEST_NCA_ID account that does not
                // have
                ...
                // Attempt to update secrets for a function of TEST_NCA_ID account with no
                ...
                Arguments.of((Supplier<String>) () -> {
                             ...
                             Set.of(SecretDto.builder().name("secret1")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.FORBIDDEN),
                // apikey has resources for TEST_FUNCTION_ID_2, but trying to access TEST_FUNCTION_ID.
                // Both apikey and function belong to same account TEST_NCA_ID
                Arguments.of((Supplier<String>) () -> {
                                 setResponse(TEST_NCA_ID, TEST_OWNER_ID,
                                             List.of(new Resource("function",
                             ...
                             Set.of(SecretDto.builder().name("secret1")
                                            .value(new StringNode("value1")).build()),
                             HttpStatus.FORBIDDEN),
                // apikey belongs to TEST_NCA_ID_2, trying to access TEST_FUNCTION_ID (belongs to
                // TEST_NCA_ID).
    ...
    void shouldUpdateFunctionSecrets(
        ...
        // Check there are no secrets and hasSecrets is false
        var secretDtos = essService.getFunctionVersionSecrets(TEST_FUNCTION_ID, TEST_VERSION_ID_1).orElse(null);
        assertThat(secretDtos).isNull();
        var functionEntity = functionsRepository.getByFunctionVersionId(TEST_VERSION_ID_1).orElseThrow();
        assertThat(functionEntity.hasSecrets()).isFalse();

        // Update the function with new secrets
        var updateFunctionRequestBody = UpdateFunctionSecretsRequest.builder()
                .secrets(secrets)
                .build();
        var url = URI.create("/v2/nvcf/secrets/functions/" + TEST_FUNCTION_ID
        ...
        // Check new secrets are actually added and hasSecrets is now true
        var newSecretDtos = essService
                .getFunctionVersionSecrets(TEST_FUNCTION_ID, TEST_VERSION_ID_1).orElse(null);
        assertThat(newSecretDtos).isNotNull().hasSize(secrets.size());
        var updatedFunctionEntity = functionsRepository.getByFunctionVersionId(TEST_VERSION_ID_1).orElseThrow();
        assertThat(updatedFunctionEntity.hasSecrets()).isTrue();
    }
