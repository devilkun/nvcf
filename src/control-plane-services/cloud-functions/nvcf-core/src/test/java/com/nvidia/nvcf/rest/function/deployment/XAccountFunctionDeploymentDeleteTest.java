...
class XAccountFunctionDeploymentDeleteTest {
    ...
    @Test
    void deleteFunctionDeploymentUsesWorkloadDelete() {
        createDeployment(
                TEST_NCA_ID,
                TEST_FUNCTION_ID,
                TEST_VERSION_ID_1,
                TEST_DEPLOYMENT_ID,
                TEST_FUNCTION_NAME);
        MockIcmsServer.getMockIcmsServer().resetRequests();

        var responseEntity = deleteDeployment(
                TEST_NCA_ID, TEST_FUNCTION_ID, TEST_VERSION_ID_1);

        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
        var requests = MockIcmsServer.getMockIcmsServer().getAllServeEvents();
        assertThat(requests).anyMatch(event -> isDeleteTo(
                event, "/v1/si/accounts/" + TEST_NCA_ID + "/workloads/" + TEST_DEPLOYMENT_ID));
    }
    ...
    private void createDeployment(
            ...
            UUID deploymentId,
            String functionName) {
        testService.createTestFunctionEntity(functionId, functionVersionId, ncaId, functionName,
                                             FunctionStatus.DEPLOYING);
        testService.createDeploymentEntity(functionId, functionVersionId, deploymentId, ncaId);
    }

    private ResponseEntity<FunctionResponse> deleteDeployment(
            String ncaId,
            UUID functionId,
            UUID functionVersionId) {
        ...
        var requestEntity = RequestEntity.delete(
                ...
                .build();
        return testRestTemplate.exchange(requestEntity, FunctionResponse.class);
    }

    private static boolean isDeleteTo(ServeEvent event, String url) {
        var request = event.getRequest();
        return "DELETE".equals(request.getMethod().getName())
