...
record ReportingServiceValue(int code) implements ReportingService {
    ReportingServiceValue {
        var unusedBits = BitMaskUtil.extractAllExceptFirst(code, BITS);
        if (unusedBits != 0) {
            var msg = "Only [%d] bits can be used to define a reporting service.".formatted(BITS);
