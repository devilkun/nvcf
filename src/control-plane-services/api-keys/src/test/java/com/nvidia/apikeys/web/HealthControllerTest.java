...
import static com.nvidia.apikeys.utils.JsonUtils.getRequestResponseJsonMapper;
import static com.nvidia.apikeys.utils.TestUtils.assertJsonEquals;
...
class HealthControllerTest {
    ...
    @Test
    void externalHealthTest() throws JacksonException {
        ResponseEntity<String> result = template.getForEntity("/health", String.class);
        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertJsonEquals(getRequestResponseJsonMapper(), "{\"status\":\"UP\"}", result.getBody());
    }

    @Test
    void internalHealthTest() throws JacksonException {
        ResponseEntity<String> result = template.getForEntity(
                "http://localhost:" + managementPort + "/admin/health", String.class);
        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertJsonEquals(
                getRequestResponseJsonMapper(),
