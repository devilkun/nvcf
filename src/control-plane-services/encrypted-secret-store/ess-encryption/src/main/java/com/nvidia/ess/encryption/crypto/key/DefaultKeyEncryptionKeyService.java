...
/**
 * Preparing a separate implementation in case either only one of SMS or SCS can go to Prod with Org
 * Key changes
 */
public class DefaultKeyEncryptionKeyService
    ...
    public Mono<OctetSequenceKey> getDecryptionKey(String namespace, String kid) {
        if (!defaultKeyProperties.getParsedAllDefaultKeys().containsKey(kid)) {
            return Mono.error(() -> new MissingKeyException(
                    "Decryption key does not exist corresponding to kid"));
        }
