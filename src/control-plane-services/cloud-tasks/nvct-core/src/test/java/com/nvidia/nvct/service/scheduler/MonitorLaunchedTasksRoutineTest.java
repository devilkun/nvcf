...
class MonitorLaunchedTasksRoutineTest {

    @Autowired
    private JsonMapper jsonMapper;

    @Autowired
    private MonitorLaunchedTasksRoutine monitorLaunchedTasksRoutine;
    ...
    void beforeAll() {
        ...
        monitorLaunchedTasksRoutine.setEnabled(true);
    }
    ...
    void cleanup() {
        ...
        MockIcmsServer.stop();
        MockCasServer.stop();
        monitorLaunchedTasksRoutine.setEnabled(false);
    ...
    void testNonLaunchedTaskRun() {
        ...
        monitorLaunchedTasksRoutine.runUnchecked(Instant.now());
    ...
    void testNoCapacityRun() {
        ...
        testTaskService.createTask(TEST_NCA_ID, TEST_TASK_ID_1, TEST_ICMS_REQ_ID_1);
        tasksRepository.findById(TEST_TASK_ID_1);
        // make sure current task status is LAUNCHED
        var taskEntity = taskService.updateTask(TEST_TASK_ID_1, LAUNCHED);
