...
import static com.nvidia.nvct.util.NvctConstants.ADMIN_SCOPE_UPDATE_SECRETS;
...
class XAccountSecretManagementControllerTest {
    ...
    Stream<Arguments> updateSecretArgs() {
        ...
        return Stream.of(
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                             ...
                             HttpStatus.BAD_REQUEST),
                // update secret for task that belongs to a different account
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                                                             List.of(ADMIN_SCOPE_UPDATE_SECRETS),
                                                             100),
                             TEST_NCA_ID_2,
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                             ...
                             TEST_NCA_ID,
                             Set.of(SecretDto.builder().name("secret1").value(new StringNode("value1")).build(),
                                    SecretDto.builder().name("secret1").value(new StringNode("value2")).build()),
                             ResultHandlingStrategy.UPLOAD,
                             HttpStatus.BAD_REQUEST),
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                                                             ...
                                                             100),
                             TEST_NCA_ID,
                             Set.of(SecretDto.builder().name("").value(new StringNode("value1")).build()),
                             ResultHandlingStrategy.NONE,
                             HttpStatus.BAD_REQUEST),
                // long secret name - exceeds MAX_SECRET_NAME_LENGTH char
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                                                             List.of(ADMIN_SCOPE_UPDATE_SECRETS),
                                                             100),
                             TEST_NCA_ID,
    ...
    void shouldUpdateForTasksWithSecrets(
        ...
        essService.saveSecrets(TEST_TASK_ID_1,
                               Set.of(SecretDto.builder().name("test").value(new StringNode("value1")).build()));
        ...
        var updateRequestBody = UpdateSecretsRequest.builder()
                .secrets(secrets)
                .build();
        var url = URI.create("/v1/nvct/accounts/" + ncaId + "/secrets/tasks/" + TEST_TASK_ID_1);
    ...
    void shouldNotUpdateForTasksWithoutSecrets() {
        var tokenSupplier = MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_ADMIN_SUBJECT,
                                                            List.of(ADMIN_SCOPE_UPDATE_SECRETS),
                                                            100);
        var token = getToken(tokenSupplier);
        ...
        var updateRequestBody = UpdateSecretsRequest.builder()
                .secrets(secrets)
                .build();
        var url = URI.create("/v1/nvct/accounts/" + TEST_NCA_ID + "/secrets/tasks/" + TEST_TASK_ID_1);
        var updateRequestEntity = RequestEntity.put(url)
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "Bearer " + token)
                .body(updateRequestBody);
