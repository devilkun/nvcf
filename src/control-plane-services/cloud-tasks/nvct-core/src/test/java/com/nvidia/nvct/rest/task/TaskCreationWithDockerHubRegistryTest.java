...
import static com.nvidia.boot.mock.BootTestConstants.TEST_DOCKER_HELM_CHART;
...
class TaskCreationWithDockerHubRegistryTest {
    ...
    @Value("${nvct.registries.recognized.container.docker.oauth2.base-url}")
    ...
    Stream<Arguments> CreateTaskWithDockerImageArgs() {
        return Stream.of(
                // existing docker image
                Arguments.of(TEST_DOCKER_CONTAINER_IMAGE,
                             HttpStatus.OK),
                // docker image with digest
                Arguments.of(TEST_DOCKER_CONTAINER_IMAGE_WITH_DIGEST,
                             HttpStatus.OK),
                // image doesn't exist
                Arguments.of(TEST_DOCKER_CONTAINER_IMAGE_NOT_EXISTS,
                             HttpStatus.NOT_FOUND),
                // no permission
                Arguments.of(TEST_DOCKER_CONTAINER_IMAGE_PERMISSION_DENIED,
                             HttpStatus.FORBIDDEN));
    }

    @ParameterizedTest
    @MethodSource("CreateTaskWithDockerImageArgs")
    void shouldLaunchTasksWithDockerImages(
            URI containerImage,
    ...
    Stream<Arguments> CreateTaskWithDockerHelmArgs() {
        return Stream.of(
                // existing docker helm
                Arguments.of("oci://" + TEST_DOCKER_HELM_CHART,
                             HttpStatus.OK),
                // helm doesn't exist
                Arguments.of("oci://" + TEST_DOCKER_CONTAINER_IMAGE_NOT_EXISTS,
                             HttpStatus.NOT_FOUND)
                // the rest situations are the same as docker containers since they share the
                // same url pattern
        );
    }
    @ParameterizedTest
    @MethodSource("CreateTaskWithDockerHelmArgs")
    void shouldLaunchTasksWithDockerHelm(
            URI helmUrl,
            HttpStatus expectedStatus) {
        var token = MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                    List.of(SCOPE_LAUNCH_TASK, SCOPE_LIST_TASKS),
        ...
        assertThat(responseBody.task().ncaId()).isEqualTo(TEST_NCA_ID);
        assertThat(responseBody.task().createdAt()).isNotNull();
        assertThat(responseBody.task().helmChart()).isEqualTo(helmUrl);
    }
