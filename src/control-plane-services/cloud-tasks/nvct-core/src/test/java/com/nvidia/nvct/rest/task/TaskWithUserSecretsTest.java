...
class TaskWithUserSecretsTest {
    ...
    Stream<Arguments> createTaskWithSecretsArgs() {
        ...
        return Stream.of(
                ...
                Arguments.of(MOCK_OAUTH2_TOKEN_SERVER.getJwt(TEST_CLIENT_SUBJECT,
                                                             List.of(SCOPE_LAUNCH_TASK),
                                                             100),
                             Set.of(SecretDto.builder().name(NGC_API_KEY)
    ...
    void shouldGetTaskDetailsWithSecretsByAccount(Boolean includeSecrets) {
        ...
        var endpoint = "/v1/nvct/tasks/" + taskId;
    ...
    void shouldDeleteTaskWithSecrets() {
        ...
        var requestBody = CreateTaskRequest.builder()
                ...
                .build();
        var requestEntity =
                RequestEntity.post(URI.create("/v1/nvct/tasks"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + token)
                        .body(requestBody);
        var responseEntity = testRestTemplate.exchange(requestEntity, TaskResponse.class);
