...
until kubectl get pod -l app.kubernetes.io/component=nats-box -n {{ include "nvcf-nats.namespace" . }} -o jsonpath='{.items[0].status.phase}' | grep Running; do
  echo "Waiting for nats-box pod..."
  sleep 2
done

# Get the nats-box pod name
NATS_BOX_POD=$(kubectl get pod -l app.kubernetes.io/component=nats-box -n {{ include "nvcf-nats.namespace" . }} -o jsonpath='{.items[0].metadata.name}')
...
OUTPUT=$(kubectl exec -n {{ include "nvcf-nats.namespace" . }} $NATS_BOX_POD -- nsc generate nkey --user 2>&1)
